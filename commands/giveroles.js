  ...
  execute(interaction, id, res, g) {
    ...
    req
      .get(
        process.env.API_URL +
          "user/" +
          (interaction ? interaction.member.id : id) +
          "?d"
      )
      .then((result) => {
        ...
        if (status !== 200) {
          ...
          */
          ...
          if (user.facility === thisFacility) {
            ...
            switch (user.rating_short) {
              ...
              case "S1":
                ...
                break;
              case "S2":
                roles.push("Student 2");
                break;
              case "S3":
                roles.push("Senior Student");
                break;
              case "I1":
          ...
          if (
            ...
            nickChange = true;
            member
              .setNickname(newNick, "Roles Synchronization")
          ...
          if (res)
            return res.json({
              status: "OK",
              msg: `Your roles have been assigned, ${newNick}!<br><em>${roles.join(
                ", "
              )}</em>`,
            });
          ...
          embed.setFooter(
            nickChange ? `Your new nickname is: ${newNick}` : newNick
          );
